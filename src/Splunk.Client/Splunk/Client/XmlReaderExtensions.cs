...
namespace Splunk.Client
{
    ...
    internal static class XmlReaderExtensions
    {
        ...
        public static async Task ReadEachDescendantAsync(this XmlReader reader, string name, Func<XmlReader, Task> task)
        {
            ...
            if (await reader.ReadToDescendantAsync(name).IgnoreSyncContext())
            {
                await task(reader).IgnoreSyncContext();

                while (await reader.ReadToNextSiblingAsync(name).IgnoreSyncContext())
                {
                    await task(reader).IgnoreSyncContext();
                }
            }
        ...
        {
            return valueConverter.Convert(await reader.ReadElementContentAsStringAsync().IgnoreSyncContext());
        }
        ...
        public static async Task<string> ReadResponseElementAsync(this XmlReader reader, string name)
        {
            ...
            await reader.ReadElementSequenceAsync(name).IgnoreSyncContext();
        ...
        public static async Task<bool> ReadToDescendantAsync(this XmlReader reader, string name)
        {
            ...
            while (await reader.ReadAsync().IgnoreSyncContext() && (reader.Depth > depth))
            {
        ...
        public static async Task<bool> ReadToFollowingAsync(this XmlReader reader, string name)
        {
            ...
            while (await reader.ReadAsync().IgnoreSyncContext())
            {
                if (reader.NodeType == XmlNodeType.Element && reader.Name == name)
                {
                    return true;
        ...
        public static async Task<bool> ReadToNextSiblingAsync(this XmlReader reader, string name)
        {
            ...
            do
            {
                if (!await reader.SkipSubtreeAsync().IgnoreSyncContext())
                {
                    break;
                }
        ...
        static async Task<bool> SkipSubtreeAsync(this XmlReader reader)
        {
            ...
            if ((reader.NodeType == XmlNodeType.Element) && !reader.IsEmptyElement)
            {
                ...
                while (await reader.ReadAsync().IgnoreSyncContext() && (depth < reader.Depth))
                { }
            ...
            return await reader.ReadAsync().IgnoreSyncContext();
        }
